...
function removeGreenScreen(image: HTMLImageElement) {
  ...
  for (let index = 0; index < pixels.length; index += 4) {
    ...
    // Strong chroma green becomes fully transparent.
    ...
    // Feather anti-aliased green edges instead of cutting them harshly.
    const feather = clamp01((greenDominance - 18) / 48)
    pixels[index + 3] = Math.round(a * (1 - feather * 0.94))

    // Despill: neutralize the remaining green fringe on semi-transparent edges.
...
async function installAsset({ selector, src }: ChromaAsset) {
  try {
    ...
    // If chroma-key processing fails, keep the original asset rather than hiding it.
